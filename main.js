...
// دالة التبديل بين الأدوار (مشتري/بائع)
function switchRole() {
    ...
    // إظهار إشعار
    if (newRole === 'buyer') {
        showNotification('تم التبديل إلى وضع المشتري', 'success');
        // إذا كان في صفحة بائع، التوجيه إلى الصفحة الرئيسية
        if (window.location.pathname.includes('seller-registration.html')) {
            setTimeout(() => {
                window.location.href = 'index2.html';
            }, 1000);
        }
    } else {
        showNotification('تم التبديل إلى وضع البائع', 'success');
        // إذا كان يريد الذهاب إلى صفحة البائع
        if (window.location.pathname.includes('index2.html') || 
            ...
            // السماح بالبقاء في الصفحة الحالية مع إمكانية التصفح
        }
    }
}

// تحديث نص زر التبديل بناءً على الدور الحالي
function updateRoleSwitchButton() {
    const roleBtn = document.getElementById('role-switch-btn');
    const roleText = document.getElementById('role-switch-text');
    const mobileRoleText = document.getElementById('mobile-role-switch-text');
    
    const currentRole = localStorage.getItem('userRole') || 'buyer';
    
    if (currentRole === 'seller') {
        if (roleText) {
            roleText.textContent = '🛒 تصفح كمشتري';
        }
        if (roleBtn) {
            roleBtn.title = 'التصفح كمشتري';
        }
        if (mobileRoleText) {
            mobileRoleText.textContent = '🛒 تصفح كمشتري';
        }
    } else {
        if (roleText) {
            roleText.textContent = '👤 بائع؟';
        }
        if (roleBtn) {
            roleBtn.title = 'التبديل إلى وضع البائع';
        }
        if (mobileRoleText) {
            mobileRoleText.textContent = '👤 بائع؟';
        }
    }
}

// تحديث زر التبديل عند تحميل الصفحة
document.addEventListener('DOMContentLoaded', function() {
    updateRoleSwitchButton();
});
...
window.updateRoleSwitchButton = updateRoleSwitchButton;
